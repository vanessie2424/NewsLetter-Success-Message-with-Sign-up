 ...
 export default function App() {
  ...
  return (
    <div className=" flex justify-center items-center min-h-screen bg-dark-grey font-roboto max-md:w-full ">
          { !showThankMessage ? (<div className='flex justify-center items-center p-5 bg-white-own px-2 rounded-3xl max-md:flex-col-reverse'>
        <div className='px-6 text-charcoal-grey'>
           <h1 className='pb-6 text-charcoal-grey text-[40px] font-bold'>Stay updated !</h1>
           ...
           <div>
             <form className='flex justify-center items-center flex-col gap-7' onSubmit={handleSubmit}>
               <input style={formStyle} className=' bg-white-own border-solid border-charcoal-grey border-[1.3px] rounded-md border-spacing-1.5 w-[350px] h-10 pl-3 text-charcoal-grey' type="email" placeholder={!touched.email ? 'email@company.com' : null} name='email'
                  value={values.email} onBlur={handleBlur} onChange={handleChange}/>
                  {console.log(values)}
                <button className=' bg-charcoal-grey w-[350px] h-12 text-white-own font-semibold rounded-md hover:bg-tomato' type='submit'>Subscribe to monthly newsletter</button>
